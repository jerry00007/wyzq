...
def filter_consecutive_zt_stocks(df):
    ...
    if '连板数' in df.columns:
        return df[df['连板数'] == 2].copy()  # 筛选连板数为 2 的股票并返回副本
    else:
        return pd.DataFrame()  # 如果没有连板数列，返回空 DataFrame

def calculate_fengliu_ratio(df):
    """计算风流比（封板资金/流通市值）并转换为千分比"""
    if '封板资金' in df.columns and '流通市值' in df.columns:
        df.loc[:, '风流比'] = (df['封板资金'] / df['流通市值']) * 1000  # 转为千分比
    else:
        df.loc[:, '风流比'] = None  # 如果数据列缺失，设置为空
    return df
...
def main():
    ...
    # TODAY = 2024121
    # stock_zt_df = get_zt_stocks_by_day(TODAY)
    ...
    # 计算风流比
    consecutive_zt_stocks = calculate_fengliu_ratio(consecutive_zt_stocks)

    # 按风流比降序排列
    consecutive_zt_stocks_sorted = consecutive_zt_stocks.sort_values(by='风流比', ascending=False)

    # 输出筛选后的股票（包含代码、名称、连板数、风流比）
    print(f"上一个交易日（{previous_trading_day}）连板数为 2 的股票（按风流比降序排列）：")
    print(consecutive_zt_stocks_sorted[['代码', '名称', '连板数', '封板资金', '流通市值', '风流比', '炸板次数', '涨停统计', '首次封板时间']])
    ...
    if stock_info:
        ...
        print("\n按今日涨跌幅升序排序的股票：")
        print(sorted_stocks[['代码', '名称', '涨跌幅', '换手率', '量比']])
